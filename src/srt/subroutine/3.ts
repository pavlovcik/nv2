module.exports = function SRT3(callback) { // "Spread construction."
    ...
    let parse_javascript = function parse_javascript(html_content) { //  html_content = STRING
    ...
    if (nv.selectors.spreads) {
        let Spreads = function Spreads(DOMSpreads, injectURLs) { // Spread constructor and adds each to controller.
            ...
            if (injectURLs) {
                ...
                spreadsPending = w;
                // console.log(spreadsPending , w, 327);
                while (w > ++x) {
                    ...
                    if (typeof injectURLs.injected[x].uri == "string") { // URL expected. This is the proper way to pass in injected spreads.
                        ...
                        if (!injectURLs.injected[x].data) { //  one
                            let get_spread = function get_spread(xhr) {
                                xhr.data.innerHTML = [
                                    "<article>",
                                ...
                                if (!--spreadsPending) {
                                    // console.log('spreadsPending!', spreadsPending);
                                    subroutine[6]();
                                }
                            };
